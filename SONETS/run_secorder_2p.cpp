...
int main(int argc, char *argv[]) {
  ...
  double alpha_xx = atof(argv[1]);
  double cc_xx = atof(argv[2]);
  deterministic_seed = atoi(argv[3]);

  int N_nodes[2] = {1200,300};
  double p[2][2] = {{0.2, 0.2},{0.2,0.2}};
  double alpha_recip[2][2] = {{2*alpha_xx, 0.0},{99,2*alpha_xx}};
  ...
  // diagonal entries
  double alpha_conv_hom_00 = alpha_xx;  // zeros onto zero (1,1)
  double alpha_conv_hom_01 = 0;  // ones onto zero  (2,2)//  double alpha_conv_hom_01 = 0;  // ones onto zero  (2,2)
  double alpha_div_hom_00  = alpha_xx;  // zero onto zeros (3,3)
  double alpha_div_hom_10  = alpha_xx;  // zero onto ones  (4,4)

  // cross entries for divergence and convergence
  // constrained by the diagonal entries
  double cc_conv_0_01 = 0;  // mix onto zero  (2,1)  double cc_conv_0_01 = 0;  // mix onto zero  (2,1)
  double cc_div_01_0  = 1;  // zero onto mix  (4,3)

  // remaining four cross entries are chains through population 0
  // constrained by diagonal entries
  double cc_chain_000 = cc_xx;  // (3,1)
  double cc_chain_100 = cc_xx;  // (4,1)
  ...
  // diagonal entries
  double alpha_conv_hom_10 = 0;  // zeros onto one (1,1)  double alpha_conv_hom_10 = 0;  // zeros onto one (1,1)
  double alpha_conv_hom_11 = alpha_xx;  // ones onto one  (2,2)
  double alpha_div_hom_01  = alpha_xx;  // one onto zeros (3,3)
  double alpha_div_hom_11  = alpha_xx;  // one onto ones  (4,4)

  // cross entries for divergence and convergence
  // constrained by the diagonal entries
  double cc_conv_1_01 = 0;  // mix onto one  (2,1)  double cc_conv_1_01 = 0.0;  // mix onto one  (2,1)
  double cc_div_01_1  = 1;  // one onto mix  (4,3)

  // remaining four cross entries are chains through population 1
  // constrained by diagonal entries
  double cc_chain_010 = 0;  // (3,1)
  double cc_chain_111 = cc_xx;  // (4,1)
  double cc_chain_110 = 0;  // (3,2)
  double cc_chain_011 = cc_xx;  // (4,2)
  ...
  sprintf(FNbase,"_%i_%i_%i_%1.3f_%1.3f_%1.3f_%1.3f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f_%1.2f",deterministic_seed,
	  N_nodes[0], N_nodes[1],
