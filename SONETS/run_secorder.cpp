...
int main(int argc, char *argv[]) {
  ...
  strcpy(FN, "data/");
  char seedbase[100];
  // sprintf(seedbase,"seed_%i/w",rng_seed);
  sprintf(seedbase,"seed_%i/w",rng_seed);
  strcat(FN, seedbase);
  strcat(FN, FNbase);
  strcat(FN, ".dat");
  ...
  // if ((alphahat_chain-alpha_chain)<-alpha_chain*0.05) {
  //   cout << "alpha_chain is not within 5% of the prescribed value\n";
  //   exit(-1);
  // }
  // else if ((alphahat_chain-alpha_chain)> alpha_chain*0.05){
  //   cout << "alpha_chain is not within 5% of the prescribed value\n";
  //   exit(-1);
  // }

  // if ((phat-p)<-p*0.05) {
  //   cout << "p is not within 5% of the prescribed value\n";
  //   exit(-1);
  // }
  // else if ((phat-p)> p*0.05){
  //   cout << "p is not within 5% of the prescribed value\n";
  //   exit(-1);
  // }
  ...
  strcpy(FN, "data/");
  char seedbases[100];
  sprintf(seedbases,"seed_%i/stats",rng_seed);
  strcat(FN, seedbases);
  strcat(FN, FNbase);
  strcat(FN, ".dat");
  fhnd = fopen(FN, "w");
